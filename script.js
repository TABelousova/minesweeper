...
function showfield() {
  for (var i = 0; i < n; i++) {
    for (var j = 0; j < n; j++) {
      ...
      if (
        (document.getElementById(cellname).innerText == "M") &
        (field[i][j].value != "*")
      ) {
        document.getElementById(cellname).innerText = field[i][j].value;
      ...
      if (field[i2][j2].value == "*") {
        ...
        if (field[i2][j2].value == "0") {
          ...
          if (count_cell == n * n - m) {
            ...
            document.getElementById("finish_window").style.visibility =
              "visible";
  ...
  if (n != 0) {
    ...
    if (m >= n * n) {
      document.getElementById("error_value").innerText =
        "Количество бомб должно быть меньше " + n * n;
      return false;
    } //}
    else {
      create_field();
